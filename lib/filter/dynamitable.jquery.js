    ...
    $.fn.dynamitable = function(options) {
        ...
        this.sorter = function sorter(index, order) {
           ...
           dynamitableCore.getBody().append(dynamitableCore.getRows().detach().sort(function(row_a, row_b) {
                ...
                var res = value_a.match(/([\d]{1,2})\/([\d][\d])\/([\d]{2,4})$/g);
                if( res != null )
                {
                	value_a = RegExp.$3+RegExp.$2+RegExp.$1;
                	res = value_b.match(/([\d]{1,2})\/([\d][\d])\/([\d]{2,4})$/g);
                	if( res != null ) value_b = RegExp.$3+RegExp.$2+RegExp.$1;
                	else value_b = '99999999';
                }
                else if(value_a.replace(/[^\d-]/g, '') !== '' && value_b.replace(/[^\d-]/g, '') !== '') {  // numeric order mode
